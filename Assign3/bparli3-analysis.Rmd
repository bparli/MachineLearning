...
pl15 = ggplot(wine.train, aes(wine.train$quality)) + geom_histogram(binwidth=1) + ggtitle("Cars Eval. Label Dist.")+
  theme(plot.title = element_text(lineheight=.8, face="bold", color="brown"))+
  labs(x="Wine Quality Labels", y="Total") + theme(axis.title=element_text(face="bold.italic", color="brown"))
...
Similar to some of the Random Optimization algorithms, restarts are a neccessary component of the kmeans implementation since the original cluster centers are picked at random.  It turns out, however, that in the cases of both these datasets, the minimal Sum of Squares was found very quickly, in under 10 restarts.
...
Another way we can compare the two clustering algorithms is by how they organized the data and also how that organization compares to the original label distribution.  The distribution of classification lablels is graphed alongside the two clustering algorithms for comparison.  As we can see below, the clustering algorithms differ from the car dataset distribution, although they are in agreement with each other.  The distribution appears nearly uniform for the kmeans and EM clusters while the labeled data shows its skew which was previously discussed.  Both algorithms are essentially saying many more samples are similar to each other than shown by the labels.  Of course, the algorithms do not consider labels, however, that they organize the data differently from the labeled dataset indicates the current labels may be an imperfect representation.  If there were more labels, for example, the large skew may flatten some.  

The same was done for the more difficult dataset, the Wine Quality dataset.  In this case there is less classification label distribution agreement between the EM and Kmeans algorithms.  We can see the EM graph is not as well aligned with either the Kmeans algorithm or the classification labels and again shows a fairly uniform distribution.  The Kmeans algorithm, on the other hand is in closer agreement with the labels but still not an ideal match.  In both cases, the algorithms arrive at a much more uniform distribution than the labeled data with its obvious disparity.  This again indicates a larger range of labels may have been a benefit.  Since the Wine Quality dataset proved difficult to classify, another hypothesis is there is some noise in the data in the form of mislabeled classes.            
...
The wine and Cars dataset produced the below principle components with the outputs indicating standard deviation of that component.  This is also indicative of the eigenvectors distribution since squaring each standard deviations equals a value proportional to the eigenvalue.  We can see in all the algorithms run against the Wine Quality dataset, the first critical component/projection is clearly the most important.  There is a steep decline in importance from that first component, but some of the remaining components are still needed to explain the data as the first by itself would not be enough to explain the data.   

In seeking to reduce the dimensions of the Cars dataset the first three principle compnents supply the most information.  There is a dropoff from the third to the fourth so the ideal cutoff would be at least 3 principle components for the Cars Evaluation dataset.  This is contrary to the Wine PCA output; the first three Cars principle components are close to one another in terms of standard deviation.  
...
Similarly, the Wine Quality ICA algorithm shows a very steep dropoff in variance accounted for from the first component. This ICA implementation finds independent signals by maximising entropy and conveniently also outputs the variance accounted for by each component.  From this output we can see we would need the next 5 components to explain another 50%. The outcome is similar for the Cars Evaluation ICA implementation; the first three components exaplain most of the variance, over 60% in this case.  If additional variance is needed an additional component can be added at a time since the remaining 3 all account for ~11% of the variance.  Its interesting to note the similarities between PCA and ICA here since PCA seeks to maximize variance, yet ICA seeks a transformed dataset with mutually independent features.  Yet, the profiles of PCA and ICA are similar for both the Cars Evaluation and Wine Quality datasets. 
...
The Random Projections algorithm is a different approach from the above 3 since the algorithm arrives at the desired number of features taken as input.  The algorithm was iterated over 100 times while seeking to reduce to 6 dimensions in the Wine Quality dataset and 3 in the Cars Evaluation dataset.  The Wine Quality dataset is used to gauge the effectiveness of this algorithm more below.   

The question, of course, with all these Dimensionality Reduction algorithms is does the variance explained by the reduced number of transformed dimensions explain enough of the data.  From the outputs above a logical cutoff can be identified to explain most of the variance.  However, for a dataset such as Wine Quality, which has proven difficult to model, is that enough? Will more of the compnents be neccessary even though they only contribute very little to the overall model? Only though trial and error can this be answered.  The Car Evluation dataset would also need trial and error but given it is so easily classifiable and so much of the information is contained in the first three components of PCA, ICA and SVD, we can hypothesize a reduction in dimensions would not impact the models too much.     

#Clustering after Dimensionality Reduction

The graph of Kmeans sum of squares within each cluster for the clustering performed on the reduced dimesnions is very close to the original plotted above.  Additionally, the EM algorithm arrived at the same number of clusters on the reduced dimensions compared to the original.  The bar graphs are also similar.  This all indicates these dimensionality reductions are appropriate and effective for the cars Evaluation and Wine Quality datasets.  That is, we can perform the PCA dimesionality reduction, continue with the clustering algorithms and still arrive at a similar quality of outcome.  In both cases the first 3 principle Components explained enough of the variance for the end result to be very close to the original clusters graphed above.  

The figures below show graphs of the original class labels and the two clustering algorithms after PCA Dimensionality Reduction has been performed. 
...
The PCA implementation is described in the section above, but this time using the first 6 principle components instead of 3 are used.  This will only serve to increase the variance contained in the final model.  
...
At 6 dimensions, the reduced dimensions datasets with were able to  approach the baseline error rate set in Assignment 1.  This is important to note since this dataset is difficult to classify.  It should also be noted, at least anecdotally, the training time complexity decreased as well with fewer dimensions.  As was noted and seen above, adding more dimensions from this point (6 features) yields less and less additional variance and information to the model.  If we were content with sacrificing some model performance for gains in training time and space complexity, this is an approach which would be acceptable.   

The final entry in the table is a very interesting case.  The ICA algorithm stands out from the others in that it seeks to transform the original components into a statistically different dataset rather than purely reduce dimensions.  But what if this transformation to a full set of 11 mutually independent features was modeled, similar to what is done in the Cocktail Party problem?  Surprisingly, this transformed dataset actually yielded an improvement from the original when the Neural Network was applied to it.  This experiment was run 30 times and the ICA transformed set performed better by at least 2% every time.  Since the ICA algorithm results in independent components with no shared mutual information, we can see that by transforming and separating these Wine Quality features into new independent features, we are able to gain some additional model performance. 

The variance described by the first component was discussed above and hypothesized to not provide enough information to adequately model the Wine Quality dataset.  We can see that to be the case below as the error rate continues to drop with each additional component.  This is further evidence to suggest a reduced number of dimensions is adequate with this dataset provided we are comfortable with some tradeoff.  Additionally, in the case of the Wine Quality dataset we can take this to be a reinforcement that the difficulty of the classification problem is not a result of too many dimensions.
...
* Its not addressed much in this assignment, but interesting to note nonetheless; the Neural Network time complexity decreased with the reduced dimensions.  As the number of features fed into the Neural Net algorithm fell, so too did the time to converge, although, as we saw above, the resulting classification performance suffered. 
